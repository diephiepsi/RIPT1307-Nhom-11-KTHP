...
router.post('/', auth_1.authRequired, async (req, res) => {
    ...
    await Promise.allSettled(recipients.map((u) => (0, mail_1.sendMail)({
        to: u.email,
...
router.post('/:id/vote', auth_1.authRequired, async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json(parsed.error.flatten());
    const questionId = String(req.params.id);
    const q = await db_1.prisma.question.findFirst({ where: { id: questionId, deletedAt: null }, select: { id: true } });
    if (!q)
        return res.status(404).json({ message: 'Not found' });
    const value = parsed.data.value;
    if (value === 0) {
        await db_1.prisma.vote.deleteMany({ where: { userId: req.user.id, questionId } });
    }
    else {
